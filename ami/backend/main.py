...
from ami.backend.markdown_api import app as markdown_app, MarkdownAPI
from ami.backend.log_api import app as log_app
from ami.backend.settings_api import app as settings_app
from ami.backend.ipc_service import BackendIPCService
...
async def setup_ipc():
    """Set up IPC service for backend communication."""
    global ipc_service
    if ipc_service is None:
        ipc_manager = IPCManager()
        markdown_api = MarkdownAPI()
        ipc_service = BackendIPCService(ipc_manager, markdown_api)
        await ipc_service.start()

async def cleanup_ipc():
    """Clean up IPC service."""
    global ipc_service
    if ipc_service is not None:
        await ipc_service.stop()
        ipc_service = None
