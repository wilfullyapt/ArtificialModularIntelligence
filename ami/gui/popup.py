...
class StateIndicator(QWidget):
    """Animated state indicator for different GUI states"""
    def __init__(self, parent=None):
        ...
        # State-specific colors and patterns
        self.state_configs = {
            "LISTENING": {
                "color": QColor('#4CAF50'),  # Green
                "pattern": [True] * 3,  # Pulsing dots
                "interval": 500
            },
            "THINKING": {
                "color": QColor('#2196F3'),  # Blue
                "pattern": [False] * 3,  # Moving dot
                "interval": 300
            },
            "SPEAKING": {
                "color": QColor('#FFC107'),  # Amber
                "pattern": [True] * 3,  # Wave pattern
                "interval": 400
            },
            "IDLE": {
                "color": QColor('white'),
    ...
    def set_state(self, state: str):
        ...
        if state != self.current_state:
            ...
            self.timer.setInterval(config["interval"])
            if state == "IDLE":
                self.stop()
            else:
                self.start()
    ...
    def start(self):
        """Start the animation"""
        self.timer.start(self.state_configs[self.current_state]["interval"])
    ...
    def paintEvent(self, event):
        ...
        if self.current_state == "LISTENING":
            # Pulsing effect
            opacity = abs(math.sin(self.animation_pos * math.pi / 10))
            color.setAlphaF(0.3 + opacity * 0.7)
            self.animation_pos = (self.animation_pos + 1) % 20
        elif self.current_state == "THINKING":
            # Moving dot
            pattern = [False] * 3
            pattern[self.animation_pos % 3] = True
            self.animation_pos = (self.animation_pos + 1) % 3
        elif self.current_state == "SPEAKING":
            # Wave pattern
            heights = [abs(math.sin((self.animation_pos + i) * math.pi / 4)) for i in range(3)]
            self.animation_pos = (self.animation_pos + 1) % 8
        ...
        for i, active in enumerate(pattern):
            if self.current_state == "SPEAKING":
                # Draw varying height bars for speaking
                height = 12 * heights[i]
                y = 16 - height
                painter.fillRect(10 + i*15, y, 6, height, color)
            else:
                # Draw dots for other states
                if active:
...
class MessageWidget(QWidget):
    ...
    def __init__(self, role: str, parent=None):
        ...
        # Message container for text and state indicator
        self.message_container = QWidget()
        message_layout = QHBoxLayout(self.message_container)
        message_layout.setContentsMargins(0, 0, 0, 0)
        message_layout.setSpacing(5)
        ...
        # Add state indicator widget
        self.state_indicator = StateIndicator()
        self.state_indicator.hide()

        message_layout.addWidget(self.message_label)
        message_layout.addWidget(self.state_indicator)
        message_layout.addStretch()

        layout.addWidget(self.role_label)
        layout.addWidget(self.message_container, 1)

    def setText(self, text: str, state: str = "IDLE"):
        """Update text and animation state"""
        self.message_label.setText(text)
        if state != "IDLE":
            self.state_indicator.show()
            self.state_indicator.set_state(state)
        else:
            self.state_indicator.set_state("IDLE")
            self.state_indicator.hide()

    def cleanup(self):
        """Clean up resources"""
        self.state_indicator.stop()
...
class AMIDialog(QDialog):
    ...
    def start_listening(self):
        ...
        message_widget.setText("Listening...", state="LISTENING")
    ...
    def prepare_ai_response(self):
        ...
        message_widget.setText("Thinking...", state="THINKING")
        self.message_layout.addWidget(message_widget)
    ...
    def show_ai_message(self, text: str, expand: bool = False):
        ...
        if isinstance(message_widget, MessageWidget) and message_widget.role_label.text() == "AI":
            message_widget.setText(text, state="SPEAKING")
        else:
            message_widget = MessageWidget("AI")
            message_widget.setText(text, state="SPEAKING")
            self.message_layout.addWidget(message_widget)
    ...
    def resume_listening(self):
        """Resume listening state after AI response"""
        if not self.is_active:
            return
