...
class ProcessManager:
    ...
    def start_ai(self):
        stop_event = mp.Event()
        process = mp.Process(target=subprocess_ai, args=(stop_event,), name="ai_process")
        process.start()
...
def get_args():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Boolean verbosity setting (default: False)'
    )
    parser.add_argument(
        '--wait-deps',
        '-w',
...
def dev(ai=False, gui=False, backend=False):
    print(" --- GOOD LUCK TO THE DEV ---")
    exit_code = 0
    try:
        startup_settings()
        process_manager = ProcessManager()
        ...
        if gui:
            signal.signal(
                        signal.SIGINT,
                        lambda s, f:
                            signal_handler(s, f, process_manager, QApplication)
                        )
            signal.signal(
                        signal.SIGTERM,
                        lambda s, f:
